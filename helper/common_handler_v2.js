...
exports.pdfGeneration = async function pdfGeneration(instaRes) {
  return new Promise(async function (resolve, reject) {
    ...
    try {
      ...
      instaRes.response = instaRes.response
        ? instaRes.response
        : instaRes.reportSections;

      //loop the response and store multiselect and radio questions of matrix type
      await Promise.all(
        instaRes.response.map(async (ele) => {
          if (ele.responseType == "matrix") {
            await Promise.all(
              ele.instanceQuestions.map((element) => {
      ...
      if (matrixMultiSelectArray.length > 0) {
        matrixMultiSelectChartObj = await getChartObject(
          matrixMultiSelectArray
        );
        formDataMatrixMultiSelect = await createChart(
          matrixMultiSelectChartObj,
          imgPath
        );
      }
      ...
      ejs
        .renderFile(__dirname + "/../views/header.ejs", {
          data: params,
        })
            ...
            async function (errWr, dataWr) {
              if (errWr) {
                ...
                await Promise.all(
                  instaRes.response.map(async (ele) => {
                    if (
                      ele.responseType === "text" ||
                      ele.responseType === "date" ||
                      ele.responseType === "number" ||
                      ele.responseType === "slider"
                    ) {
                      arrOfData.push(ele);
                    } else if (ele.responseType === "multiselect") {
                      ...
                      await Promise.all(
                        ele.instanceQuestions.map((element) => {
                          ...
                          if (
                            ...
                            element.responseType == "date" ||
                            element.responseType == "number" ||
                            ele.responseType == "slider"
                          ) {
                ...
                ejs
                  .renderFile(__dirname + "/../views/mainTemplate.ejs", {
                    data: obj,
                  })
                                  ...
                                  async function (err) {
                                    if (err) {
                                      ...
                                      if (uploadFileResponse.success) {
                                        let pdfDownloadableUrl =
                                          await getDownloadableUrl(
                                        ...
                                        if (
                                          ...
                                          return resolve({
                                            ...
                                            message:
                                              filesHelper.pdf_report_generated,
                                            pdfUrl:
                                              pdfDownloadableUrl.data.result
                                                .url,
                                          });
...
exports.instanceObservationPdfGeneration =
  async function instanceObservationPdfGeneration(instaRes) {
    return new Promise(async function (resolve, reject) {
      var currentTempFolder =
        "tmp/" +
        uuidv4() +
        "--" +
        Math.floor(Math.random() * (10000 - 10 + 1) + 10);
      ...
      try {
        ...
        let multiselectFormData = await createChart(
          multiSelectChartObj,
          imgPath
        );
        ...
        var params = {
          observationName: instaRes.observationName
            ? instaRes.observationName
            : instaRes.solutionName,
        };
        ejs
          .renderFile(__dirname + "/../views/header.ejs", {
            data: params,
            ...
            fs.writeFile(
              ...
              async function (errWr, dataWr) {
                if (errWr) {
                  ...
                  await Promise.all(
                    instaRes.response.map(async (ele) => {
                      if (
                        ...
                        ele.responseType === "number" ||
                        ele.responseType === "slider" ||
                        ele.responseType === "multiselect" ||
                        ele.responseType === "radio"
                      ) {
                        arrOfData.push(ele);
                      } else if (ele.responseType === "matrix") {
                        //push main matrix question object into array
                  ...
                  ejs
                    .renderFile(
                      __dirname + "/../views/instanceObservationTemplate.ejs",
                      {
                        data: obj,
                      }
                    )
...
exports.instanceObservationScorePdfGeneration =
  async function instanceObservationPdfGeneration(observationResp, obj) {
    return new Promise(async function (resolve, reject) {
      var currentTempFolder =
        "tmp/" +
        uuidv4() +
      ...
      try {
        ...
        observationResp.response = observationResp.response
          ? observationResp.response
          : observationResp.reportSections;
            ...
            fs.writeFile(
              ...
              async function (errWr, dataWr) {
                if (errWr) {
                  ...
                  await Promise.all(
                    observationResp.response.map(async (ele) => {
                  ...
                  ejs
                    .renderFile(
                      __dirname + "/../views/instanceScoreObsTemplate.ejs",
                      {
                        data: obj,
                      }
                    )
...
const getAssessmentChartData = async function (assessmentData) {
  ...
  let chartData = await convertChartDataToPercentage(
    assessmentData.domainLevelObject
  );
...
exports.assessmentAgainPdfReport = async function (assessmentResponse) {
  return new Promise(async function (resolve, reject) {
    ...
    try {
      let assessmentChartData = await getAssessmentAgainChartData(
        assessmentResponse
      );
      let chartData = await getChartObject([
        assessmentChartData.reportSections[0],
      ...
      ejs
        .renderFile(path.join(__dirname, "../views/assessment_header.ejs"), {
          data: params,
        })
        .then(function (headerHtml) {
          var dir = imgPath;
              ...
              if (errWr) {
                ...
                ejs
                  .renderFile(
                    path.join(
                      __dirname,
                      "../views/stacked_bar_assessment_template.ejs"
                    ),
                    {
                      data: obj.path[0].options.filename,
...
const getAssessmentAgainChartData = async function (assessmentData) {
  ...
  let chartData = await convertAssessAgainChartDataToPercentage(
    ...
    datalabels: {
      ...
      formatter: function (value, data) {
        if ((data.datasetIndex + 1) % chartData.length == 0) {
          ...
          j++;
          if (
            assessmentData.reportSections[0].chart.submissionDateArray.length >
            0
          ) {
            let submissionValue =
              assessmentData.reportSections[0].chart.submissionDateArray[k];
            k++;
...
exports.unnatiViewFullReportPdfGeneration = async function (responseData) {
  return new Promise(async function (resolve, reject) {
    ...
    try {
      ...
      ejs
        .renderFile(__dirname + "/../views/unnatiViewFullReport.ejs", {
          data: obj,
        })
        .then(function (dataEjsRender) {
          var dir = imgPath;
                      ...
                      fs.writeFile(
                        ...
                        async function (err) {
                          if (err) {
                            ...
                            let uploadFileResponse = await uploadPdfToCloud(
                              pdfFile,
                              dir
                            );
                            ...
                            if (uploadFileResponse.success) {
                              ...
                              if (
                                pdfDownloadableUrl.success &&
                                pdfDownloadableUrl.data.result &&
                                Object.keys(pdfDownloadableUrl.data.result)
                                  .length > 0
                              ) {
                                fs.readdir(imgPath, (err, files) => {
                                  if (err) throw err;
...
exports.instanceCriteriaReportPdfGeneration = async function (
  instanceResponse
) {
  return new Promise(async function (resolve, reject) {
    var currentTempFolder =
    ...
    try {
      ...
      let params = {
        observationName: instanceResponse.observationName
          ? instanceResponse.observationName
          : instanceResponse.solutionName,
      };
      ejs
        .renderFile(__dirname + "/../views/header.ejs", {
          data: params,
              ...
              if (err) {
                ...
                var obj = {
                  response: instanceResponse.response
                    ...
                    : instanceResponse.reportSections,
                };
                ejs
                  .renderFile(
                    __dirname + "/../views/instanceCriteriaTemplate.ejs",
                    {
                      data: obj,
                    }
                  )
  ...
  return new Promise(async function (resolve, reject) {
    ...
    try {
      ...
      responseData.response = responseData.response
        ...
        : responseData.reportSections;

      await Promise.all(
        responseData.response.map(async (singleResponse) => {
          await Promise.all(
            singleResponse.questionArray.map((question) => {
      ...
      let params = {
        observationName: responseData.observationName
          ? responseData.observationName
          : responseData.solutionName,
      };
          ...
          fs.writeFile(
            ...
            async function (errWr, dataWr) {
              if (errWr) {
                ...
                ejs
                  .renderFile(
                    __dirname + "/../views/entityCriteriaTemplate.ejs",
                    {
                      data: obj,
                    }
                  )
...
//PDF generation for instance observation score report
exports.instanceScoreCriteriaPdfGeneration = async function (
  observationResp,
  obj
) {
  return new Promise(async function (resolve, reject) {
    ...
    try {
      ...
      await Promise.all(
        observationResp.response.map(async (questionData) => {
          await Promise.all(
            questionData.questionArray.map(async (singleQuestion) => {
      ...
      let params = {
        observationName: observationResp.observationName
          ? observationResp.observationName
          : observationResp.solutionName,
      };
          ...
          fs.writeFile(
            ...
            async function (errWr, dataWr) {
              if (errWr) {
                ...
                ejs
                  .renderFile(
                    __dirname + "/../views/scoreCriteriaTemplate.ejs",
                    {
                      data: obj,
                    }
                  )
    ...
    await Promise.all(
      projects.map(async (eachProject) => {
        ...
        if (data.length > 0) {
          data.forEach((v) => {
            leastStartDate =
              new Date(v.startDate) < new Date(leastStartDate)
                ? v.startDate
                : leastStartDate;
          });
        }
...
exports.unnatiEntityReportPdfGeneration = async function (entityReportData) {
  return new Promise(async function (resolve, reject) {
    ...
    try {
      ...
      ejs
        .renderFile(__dirname + "/../views/unnatiEntityReport.ejs", {
          data: ejsInputData,
        })
        .then(function (dataEjsRender) {
          let dir = imgPath;
...
const uploadPdfToCloud = async function (fileName, folderPath) {
  return new Promise(async function (resolve, reject) {
    try {
      let getSignedUrl = await kendraHelper.getPreSignedUrl(fileName);

      if (getSignedUrl.result && Object.keys(getSignedUrl.result).length > 0) {
        let fileUploadUrl =
          getSignedUrl.result[Object.keys(getSignedUrl.result)[0]].files[0].url;
        ...
        try {
          await request({
            ...
            headers: {
              "x-ms-blob-type":
                getSignedUrl.result[Object.keys(getSignedUrl.result)[0]]
                  .files[0].cloudStorage == filesHelper.azure
                  ? "BlockBlob"
                  : null,
          ...
          return resolve({
            success: true,
            data: getSignedUrl.result[Object.keys(getSignedUrl.result)[0]]
              .files[0].payload.sourcePath,
          });
        } catch (e) {
          console.log(e);
        }
...
const getDownloadableUrl = async function (filePath) {
  return new Promise(async function (resolve, reject) {
    try {
      let response = await kendraHelper.getDownloadableUrl(filePath);
...
exports.questionResponseReportPdf = async function (dataObj) {
  return new Promise(async function (resolve, reject) {
    try {
      ...
      await Promise.all(
        dataObj.responseData.map(async (domainElement) => {
          await Promise.all(
            domainElement.criterias.map(async (criteriaElement) => {
              await Promise.all(
                criteriaElement.questionData.map(
                  async (questionDataElement) => {
                    const options = await createChartOption(
                      questionDataElement
                    );
                    const questionResponseChart = await createChart(
                      options.chartOptions,
                      imgPath
                    );
      ...
      await Promise.all(
        dataObj.responseData.map(async (domainElement) => {
          await Promise.all(
            domainElement.criterias.map(async (criteriaElement) => {
              if (
                criteriaElement.questionData &&
                criteriaElement.questionData.length > 0
              ) {
                let myArray = criteriaElement.questionData;
                myArray.sort(
                  (a, b) => a.questionSequenceByEcm - b.questionSequenceByEcm
                );
      ...
      ejs
        .renderFile(
          __dirname + "/../views/questionResponseReportTemplate.ejs",
          {
            data: dataObj,
          }
        )
...
const createChartOption = async function (questionData) {
  try {
    ...
    if (questionData.answerData.length > 0) {
      await Promise.all(
        questionData.answerData.map((answerElement) => {
    ...
    if (
      questionData.questionResponseType == "radio" ||
      questionData.questionResponseType == "slider"
    ) {
      let yValuesNew = [];
      for (let i = 0; i < yValues.length; i++) {
...
exports.coreStatsReportGeneration = async function (overview) {
  return new Promise(async function (resolve, reject) {
    ...
    try {
      ...
      const userOverViewResponseChart = await createChart(
        options.chartOptions,
        imgPath
      );
      let FormData = [];
      ...
      ejs
        .renderFile(__dirname + "/../views/coreStatsReport.ejs", {
